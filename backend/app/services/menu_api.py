...
from app.services.weather_service import get_weather_summary_for_menu  # ★ NEW
...
def get_weekly_weather(week_start: str = None):
    """주간 날씨 예보 조회 (식단 페이지용)"""
    if week_start:
        try:
    ...
    weather = get_weather_summary_for_menu(ws)
    return {"week_start": ws.isoformat(), "weather": weather}
