...
from app.services.weather_service import (
    fetch_weekly_weather, get_weather_for_date, calculate_weather_score
)

# ─── 팬트리 재료 (장보기에서 분리) ──────────
PANTRY_CATEGORIES = {"양념"}
PANTRY_INGREDIENTS = {
    "간장", "된장", "고춧가루", "고추장", "설탕", "소금", "식용유",
    "참기름", "들기름", "식초", "맛술", "다진마늘", "후추", "맛술",
    "굴소스", "국간장", "깨", "참깨", "들깨"
}

# ─── 주재료 단백질 태그 ──────────────────────
PROTEIN_TAGS = {"돼지고기", "소고기", "닭고기", "생선", "해산물", "달걀", "두부"}
...
def _score_recipe(recipe: Recipe, selected: list[dict], current_date: date,
                  meal_type: str, user_pref: UserPreference | None,
    ...
    # ★ 5) 날씨/계절 기반 점수 (NEW)
    weather_score = calculate_weather_score(recipe_tags, weather, current_date)
    score += weather_score
...
def generate_weekly_menu(db: Session, user_id: int, week_start: date) -> MenuPlan:
    """
    주간 메뉴 자동 생성 (아침/점심/저녁 × 7일)
    ★ 날씨 데이터를 활용하여 기온/강수/계절에 맞는 메뉴 추천
    """
    ...
    for day_offset in range(7):
        ...
        # ★ 해당 날짜의 날씨 데이터 가져오기
        day_weather = get_weather_for_date(weather_data, current_date)
        ...
        for meal_type_str in ["BREAKFAST", "LUNCH", "DINNER"]:
            ...
            for recipe in sample:
                # ★ 날씨 정보를 scoring에 전달
                score = _score_recipe(
                    recipe, selected_items, current_date,
                    meal_type_str, user_pref,
                    weather=day_weather  # ★ NEW
                )
                if score < best_score:
                    best_score = score
                    best_recipe = recipe
...
def replace_single_meal(db: Session, menu_plan_item_id: int) -> MenuPlanItem:
    ...
    # ★ 교체 시에도 날씨 고려
    weather_data = fetch_weekly_weather()
    day_weather = get_weather_for_date(weather_data, item.date)
    ...
    # ★ 날씨 점수 적용하여 최적 선택
    if candidates and day_weather:
        scored = []
        for r in candidates:
            tags = set(r.tags or [])
            w_score = calculate_weather_score(tags, day_weather, item.date)
            scored.append((r, w_score + random.uniform(0, 5)))
        scored.sort(key=lambda x: x[1])
        new_recipe = scored[0][0]  # 가장 적합한 레시피
    else:
