...
def signup(req: SignupRequest, db: Session = Depends(get_db)):
    ...
    user = User(email=req.email, hashed_password=hash_password(req.password), name=req.name, birth_year=req.birth_year, sex=req.sex, height_cm=req.height_cm, weight_kg=req.weight_kg, activity_level=req.activity_level)
    db.add(user)
    db.flush()
    ...
    return TokenResponse(access_token=token, user=UserOut(id=user.id, name=user.name, birth_year=user.birth_year, sex=user.sex.value if user.sex else None, height_cm=user.height_cm, weight_kg=user.weight_kg, activity_level=user.activity_level, kcal_target=kcal))
...
@router.get("/kakao/login", summary="카카오 로그인")
def kakao_login():
...
@router.get("/kakao/callback", summary="카카오 콜백")
async def kakao_callback(code: str, db: Session = Depends(get_db)):
    post_data = {"grant_type": "authorization_code", "client_id": KAKAO_REST_API_KEY, "redirect_uri": KAKAO_REDIRECT_URI, "code": code, "client_secret": KAKAO_CLIENT_SECRET}
    async with httpx.AsyncClient() as client:
        token_res = await client.post("https://kauth.kakao.com/oauth/token", data=post_data, headers={"Content-Type": "application/x-www-form-urlencoded"})
    if token_res.status_code != 200:
        return RedirectResponse(url="/static/?error=" + token_res.text)
    kakao_token = token_res.json().get("access_token")
    async with httpx.AsyncClient() as client:
        user_res = await client.get("https://kapi.kakao.com/v2/user/me", headers={"Authorization": "Bearer " + kakao_token})
    if user_res.status_code != 200:
        return RedirectResponse(url="/static/?error=kakao_user_failed")
    ...
    if not user and email:
        ...
        if user:
            ...
            db.commit()
    if not user:
        user = User(kakao_id=kakao_id, email=email, name=nickname, hashed_password=None)
        db.add(user)
        db.flush()
    ...
    return RedirectResponse(url="/static/?token=" + jwt_token + "&user_id=" + str(user.id) + "&user_name=" + nickname + "&user_kcal=" + str(kcal))


@router.get("/me", response_model=MeResponse, summary="내 정보")
def get_me(user: User = Depends(require_login)):
    kcal = user.preferences.kcal_target if user.preferences else None
    return MeResponse(id=user.id, email=user.email, name=user.name, birth_year=user.birth_year, sex=user.sex.value if user.sex else None, height_cm=user.height_cm, weight_kg=user.weight_kg, activity_level=user.activity_level, kcal_target=kcal)
